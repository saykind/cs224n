...
We then initialize the \textbf{decoder}'s first hidden state $\hdec_0$ and cell state $\cdec_0$ with a linear projection of the encoder's final hidden state and final cell state.\footnote{If it's not obvious, think about why we regard $[\hencfw{m}, \hencbw{1}]$ as the `final hidden state' of the Encoder.} 
...
We now concatenate the decoder hidden state $\hdec_t$ with the attention output $\ba_t$ and pass this through a linear layer, tanh, and dropout to attain the \textit{combined-output} vector $\bo_{t}$.
...
Follow the instructions in the \href{https://docs.google.com/document/d/1hNR4yuhQ4aw9k0SpyuRf6V2Ze4Bekwg6O_kBuRLjCQ0/edit#heading=h.4tqnggp12z76}{CS224n: GCP Guide 2024} (link also provided on website and Ed) in order to create your VM instance. This should take you approximately 60 minutes. Though you will need the GPU to train your model, we strongly advise that you first develop the code locally and ensure that it runs, before attempting to train it on your VM. GPU time is expensive and limited. It takes approximately \textbf{1.5 to 2 hours} to train the NMT system. We don't want you to accidentally use all your GPU time for debugging your model rather than training and evaluating it. Finally, \textbf{make sure that your VM is turned off whenever you are not using it.}

\textbf{\textcolor{red}{If your GCP subscription runs out of money, your VM will be temporarily locked and inaccessible. If that happens, please make a private Ed post describing the situation.}}
...
\end{lstlisting}
    ...
    \ifans{
    	Applying this mask, sets values of $\be_t$ corresponding to padded words (outside of the sentence) to $-\infty$. After applying softmax this results in zero values for elements of $\alpha_t$ (See eq. (8)). Finally, as can be seen from eq. (9) this leads to complete ignore of enconding of padded words: $\bh_i^\text{enc}$ is ignored in attention computation for $i$ larger than sentence length.
    	
    	This is done in to ignore the influence of padding words when computing the attention scores. If this isn't done, decoder will use encodings of padded words, which may lead to unexpected results.}
\end{parts}
...
You should see a significant decrease in loss during the initial iterations. Once you have ensured that your code does not crash (i.e. let it run till \texttt{iter 10} or \texttt{iter 20}), power on your VM from the Google Cloud Console. Then read the \textit{Transferring Files From Your Instance To Your Computer} section of the \href{https://docs.google.com/document/d/1hNR4yuhQ4aw9k0SpyuRf6V2Ze4Bekwg6O_kBuRLjCQ0/edit#heading=h.nh2g1xmhesvv}{CS224n: GCP Guide 2024} (link also given on website and Ed) for instructions on how to upload your code to the VM.
...
Finally, as suggested in the \textit{Other Tips} section of the GCP Guide, create a new \href{https://linuxize.com/post/getting-started-with-tmux/}{tmux} session. Concretely, run the following command to create tmux session called \texttt{nmt}. 
\begin{lstlisting}
    tmux new -s nmt
\end{lstlisting}
...
\end{lstlisting}
    ...
    \ifans{
    \begin{subparts}
    	\subpart[2] Dot product attention.
    	\begin{itemize}
    		\item Advantage: dot product attention is computationally efficient and simpler to implement compared to multiplicative attention.
    		\item Disadvantage: dot product attention can be sensitive to the scale of the input vectors, which can lead to issues with vanishing or exploding gradients during training.
    	\end{itemize}
    	\subpart[2] Additive attention.
    	\begin{itemize}
    		\item Advantage: Additive attention is more flexible in modeling complex relationships between query and key vectors because it uses learnable parameters to compute the attention scores.
    		\item Disadvantage: Additive attention introduces additional learnable parameters, making the model more complex and potentially harder to train.
    	\end{itemize}
    \end{subparts}
    }
\end{parts}
