    ...
    \part[4] Adam Optimizer\newline
        ...
        \begin{subparts}
            ...
            \subpart[2]First, Adam uses a trick called {\it momentum} by keeping track of $\bm$, a rolling average of the gradients:
                ...
                \begin{shaded}
               	\begin{answer}
                	After $m$ steps gradient are averaged $m$ times (through the process known as exponential moving average) which leads to a more smooth and stable convergence. Low gradient variance is helpful since it leads to faster convergence.
               	\end{answer}
                \end{shaded}
            ...
            \subpart[2] Adam extends the idea of {\it momentum} with the trick of {\it adaptive learning rates} by keeping track of  $\bv$, a rolling average of the magnitudes of the gradients:
                ...
                \begin{shaded}
                	\begin{answer}
                		Term $v$ represents the exponentially moving average of past squared gradients. By dividing by $\sqrt{v}$ variance is further decreased. Compenets of $\btheta$ which have large variance get penalized more than compopents with small variance, so that all components have roughly the same variance.
                	\end{answer}
                \end{shaded}
          ...
          \begin{shaded}
          \begin{answer}
          	i. Let's compute expectation value for $\bn_\text{drop}$.
          	\begin{align}
          		\mathbb{E}_{p_{\text{drop}}}[\bh_\text{drop}]_i = \gamma(1-p)h_i = h_i.
          	\end{align}
          	We see that $\gamma = (1-p)^{-1}$.
          	
          	ii.  Dropout is a regularization technique that helps to prevent overfitting, hence they should be used to make neural network generalization ability better. Somtimes, dropout regularization should be avoided since it adds randomness which may be undesired and it also make training more computationaly costly.
          \end{answer}
          \end{shaded}
