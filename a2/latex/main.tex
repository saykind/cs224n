...
\newcommand{\hmwkTitle}{word2vec (49 pt) by D. Saykin} % Assignment title
\newcommand{\hmwkClass}{CS\ 224n a2} % Course/class
...
%More generally, $\nabla J = 0$ when $\hat{\bm y} - \bm y$ is in the subspace of right-kernel of $U$, dimension of this space is at least $|V|-d$ which is positive when vocabulary size is larger then embedding dimension. In practice, $|V| \ll d$ hence $U$ has a largre-dimensional right-Kernel. At the same time it's clear that random vector would not fall into this Kernel. If $d=100$ and $|V|=10,000$ the probability of random vector with i.i.d. normal elements and std $=1$ to fall into the Kernel with $\epsilon = 10^{-10}$ accuracy is approximately $|V|\epsilon^d$.
...
\begin{shaded}
\begin{answer}
i. Let me introduce scalar $z_w = \bm u^\top_{w}\bm v_c$. Then loss function and its derivative
\begin{align}
    J &= -\log \sigma(z_o) - \sum_s \log \sigma(-z_{w_s})	\\
    \frac{\partial J}{\partial z_w} 
    &= -\frac{\sigma'(z_o)}{\sigma(z_o)}\delta_{ow} + \sum_s \frac{\sigma'(-z_{w_s})}{\sigma(-z_{w_s})} \delta_{w_sw}
    = \begin{cases}
        -\sigma(-z_w),	&	w=o	\\
        \sigma(z_w),	&	w \neq o
    \end{cases}
    %= -\sigma(-z_w)\delta_{ow} + \sigma(z_w)(1-\delta_{ow})
\end{align}
Here $\delta_{ij}$ is \href{https://en.wikipedia.org/wiki/Kronecker_delta}{Kroneker delta} symbol, i.e. identity matrix with shape (K+1, K+1). Using formula above
\begin{align}
    \frac{\partial J}{\partial \bm u_o} &= -\sigma(-z_o) \bm v_c
        = -\sigma(-\bm u^\top_{o}\bm v_c) \bm v_c,	\\
    \frac{\partial J}{\partial \bm u_{w_s}} &= \sigma(z_{w_s}) \bm v_c
        = \sigma(\bm u^\top_{w_s}\bm v_c) \bm v_c,	\\
    \frac{\partial J}{\partial \bm v_c} &= -\sigma(-z_o) \bm u_o + \sum_s \sigma(z_{w_s}) \bm u_{w_s}	\\
        &= -\sigma(-\bm u^\top_{o}\bm v_c) \bm u_o + \sum_s \sigma(\bm u^\top_{w_s}\bm v_c) \bm u_{w_s}.
\end{align}
Shape convention was used here.

ii. We could reuse $(K+1)$--vector $\sigma(-\bm U^\top \bm v_c)$, where $\bm U  = \bm{U}_{o, \{w_1, \dots, w_K\}}$. 

iii. In naive softmax we had to compute sum $\sum_w \exp(\bm u_w^\top \bm v_c)$, which takes $|V| \times d$ operations where $|V|$ is vocabulary size (large number) and $d$ is embedding dimension. Here most time-consuming computation is $\bm U^\top \bm v_c$, which take only $K \times d$ operations.
\end{answer}
\end{shaded}
...
\begin{shaded}
\begin{answer}
    Derivative with respect to $\bm v_c$ and $\bm u_o$ are the same as before.
\begin{align}
    \frac{\partial J}{\partial \bm v_c} &= -\sigma(-\bm u^\top_{o}\bm v_c) \bm u_o + \sum_s \sigma(\bm u^\top_{w_s}\bm v_c) \bm u_{w_s}, \\
    \frac{\partial J}{\partial \bm u_o} &= -\sigma(-\bm u^\top_{o}\bm v_c) \bm v_c.
\end{align}
If $w_s$ appearing $r_s$ times in the set, then loss function is gonna have $r_s$ identical terms in the sum. Let $\sum'$ stand for sum over such idecies that $w_s$ are distinct. 
\begin{align}
    J &= -\log(\sigma(\bm u_o^\top \bm v_c)) - {\sum_{s}}' r_s\log(\sigma(-\bm u_{w_s}^\top \bm v_c)),	\\
    \frac{\partial J}{\partial \bm u_{w_s}} &= r_s\sigma(\bm u^\top_{w_s}\bm v_c) \bm v_c.
\end{align}
\end{answer}
\end{shaded}
