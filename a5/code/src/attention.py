...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # nh : number of heads, hs: hidden size, nh * hs = C
...
class BilinearSelfAttention(nn.Module):
    """
    A multi-head masked self-attention layer with a projection at the end.
    I believe I could reduce number of parametrers by using a bilinear layer
    instead of a linear layer for the key and query projections.
    """

    def __init__(self, config):
        super().__init__()
        assert config.n_embd % config.n_head == 0
        # key and query are represented by single C-by-C matrix, where C = n_embd is embedding dimensionality
        self.key = nn.Linear(config.n_embd, config.n_embd)
        # value projections for all heads
        self.value = nn.Linear(config.n_embd, config.n_embd)
        # regularization
        self.attn_drop = nn.Dropout(config.attn_pdrop)
        self.resid_drop = nn.Dropout(config.resid_pdrop)
    ...
    def forward(self, x):
        ...
        # causal self-attention should be (B, nh, T, T)
        att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))
